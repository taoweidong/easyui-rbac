...
public class AttachmentService {
	 ...
	 */
	@Transactional
	public Attachment saveFile(Member member, AttachmentType type, MultipartFile multipartFile)
			throws Exception {

		// 判断文件是否允许呗
		if (this.getAllowFileType()
			...
			try {
				// 将文件存储到实际存储目录中
				File file = new File(this.getRootPath(), filePath);
				multipartFile.transferTo(file);

				att.setFilePath(file.getCanonicalPath());
	...
	public void clearAvatar(Member member) {
		...
		// 获取到非当前头像的历史头像记录
		List<Attachment> avatars = attachmentDao.findByFilePathIsNotAndMemberAndType(
				member.getAvatar(), member, AttachmentType.AVATAR);
		// 物理删除
		avatars.forEach((Attachment item) -> {
			try {
				FileUtils.forceDelete(new File(getRootPath(),
						item.getFilePath().substring("/attachment".length())));
			} catch (IOException e) {
